...
//		loadLstCentriDemat('centroDemat');		
	...
	$('#ASRModal').on('shown.bs.modal', function () {
	...
	$("#esportaCSV").click(function() {
		...
		var centroDemat  = $('input[name="centrodemat"]').val();
			var uri = contextPath + "/scatole/getLstScatoleAnomSanDaRiscan.json?idCentroDemat="+centroDemat+"&sort=scatola_id&order=desc&offset=0&limit=1000";
		$.ajax({
			type: 'GET',
            url: uri,
					...
					for(i = 0; i < result.rows.length; i++) {
						...
						console.log(row);
				    		...
				    		csvrow ='"' + row["IDENTIFICATIVO_SCATOLA"] + '"' + ' ;' +
				    		    '"' + row["DATA_CREAZIONE_SCATOLA"] + '"'+ ' ;'+ '"'  + row["TIPOLOGIA_SCATOLA"] + '"' + ' ;'+ '"' + row["TOTALE_ANOMALIE"] + '"' + ' ;' +
				    		    '"' + row["DATA_CHIUSURA_SCATOLA"] + '"';
...
function confermaRiscansionata(idScatola, identificativoScatola) {
	...
	var url = contextPath + '/scatole/updateScatoleAnomSanDaRiscan';
...
function caricaTabella(){
	...
	var centroDemat  = $('input[name="centrodemat"]').val();
	var uri = contextPath + "/scatole/getLstScatoleAnomSanDaRiscan.json?idCentroDemat="+centroDemat;
